...
def preprocess_data(df):
    ...
    X_code = vectorizer.fit_transform(df["code"])

    # Complexity score
    X_complexity = df[["complexity"]].values

    # Combine features
    X_combined = sp.hstack([X_code, sp.csr_matrix(X_complexity)]).tocsr()
...
def main():
    ...
    for item, pred in zip(data, all_y_pred):
        item["code_BOW_complexity_prediction"] = pred
        combined_results.append(item)

    with open(os.path.join(path, "code_BOW_complexity_predictions.json"), "w", encoding="utf-8") as f:
